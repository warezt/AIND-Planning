...
class PlanningGraph():
        ...
        for action in self.all_actions:
            ...
            if node_a.prenodes.issubset(self.s_levels[level]):
                ...
                for node_s in self.s_levels[level]:
                    ...
                    # Put node_s to be parent of node_a
        # Build the action level from the reachable action nodes
        self.a_levels.append(addableaction)
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
            ...
            for node_s in node_a.effnodes:
                ...
                addableliteral.add(node_s)
                node_s.parents.add(node_a)
                # Connect action node as parent of this node
                node_a.children.add(node_s)
                # Put node_s to be children of node_a
        self.s_levels.append(addableliteral)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #Check if node_a1 add effect is not listed in negative precondition of node_a2
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.precond_neg:
                return True
        #Check if node_a2 add effect is not listed in negative precondition of node_a1
        for effect in node_a2.action.effect_add:
            if effect in node_a1.action.precond_neg:
                return True
        #Check if node_a1 remove effect is not listed in positive precondition of node_a2
        for effect in node_a1.action.effect_rem:
            if effect in node_a2.action.precond_pos:
                return True
        #Check if node_a2 remove effect is not listed in positive precondition of node_a1
